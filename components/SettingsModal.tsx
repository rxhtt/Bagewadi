...
const SettingsModal: React.FC<Props> = ({ 
  ...
  const handleAddKey = async () => {
    ...
    try {
      if (activeTab === 'intelligence') {
        const isValid = await gemini.validateKey(newKey.trim());
        if (isValid) {
          onSaveKeys([...apiKeys, newKey.trim()]);
          setNewKey('');
        } else {
          setValidationError('Validation failed. Ensure the Gemini API key is correct and active.');
        }
  ...
  const handleRemoveKey = (provider: 'intelligence' | 'youtube', index: number) => {
  ...
  const tabs = [
    { id: 'intelligence', label: 'Intelligence', desc: 'Gemini / Search', icon: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 2L14.4 9.6L22 12L14.4 14.4L12 22L9.6 14.4L2 12L9.6 9.6L12 2Z"/></svg> },
      ...
      <div className="w-full max-w-5xl bg-[#1c1c1e] border border-white/10 rounded-[3rem] shadow-[0_0_120px_rgba(0,0,0,1)] overflow-hidden flex flex-col md:flex-row h-[700px]">
        ...
        <main className="flex-1 flex flex-col p-12 overflow-hidden bg-gradient-to-br from-transparent to-zinc-900/20">
              ...
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
                <div className="p-10 bg-zinc-900/60 rounded-[3rem] border border-white/5 flex items-center justify-between shadow-2xl">
                  <div className="space-y-3">
                    <h4 className="text-lg font-black text-white uppercase tracking-widest">Global Secure Tunnel</h4>
                  ...
                  <button 
                    ...
                    className={`relative inline-flex h-10 w-20 items-center rounded-full transition-all duration-700 ${vpnEnabled ? 'bg-blue-600 shadow-[0_0_30px_rgba(37,99,235,0.5)]' : 'bg-zinc-800'}`}
                  >
                    <span className={`inline-block h-7 w-7 transform rounded-full bg-white transition-all duration-500 ${vpnEnabled ? 'translate-x-11 shadow-2xl' : 'translate-x-1.5'}`} />
                  </button>
                </div>
              </div>
            ) : (
              <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
                <div className="space-y-6">
                  <label className="text-[11px] font-black text-zinc-600 uppercase tracking-[0.4em]">
                    {activeTab === 'intelligence' ? 'Gemini API Node Key' : 'YouTube Infra Key'}
                  </label>
                  <div className="flex flex-col sm:flex-row gap-4">
                    <input
                      ...
                      placeholder={`Inject ${activeTab === 'intelligence' ? 'Gemini' : 'YouTube'} key...`}
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-black text-zinc-700 uppercase tracking-widest">
                       {(activeTab === 'intelligence' ? apiKeys : youtubeKeys).length} Nodes Active
                    </span>
                  </div>
                  <div className="space-y-4">
                    {(activeTab === 'intelligence' ? apiKeys : youtubeKeys).map((key, idx) => (
                      <div key={idx} className="flex items-center justify-between p-6 bg-zinc-900/60 border border-white/5 rounded-[2rem] group transition-all hover:bg-zinc-800 hover:border-blue-600/30 shadow-xl">
                        ...
                        <button 
                          onClick={() => handleRemoveKey(activeTab === 'intelligence' ? 'intelligence' : 'youtube', idx)} 
          ...
          <footer className="mt-12 pt-10 border-t border-white/5 flex justify-end">
             <button onClick={onClose} className="px-14 py-5 bg-white text-black hover:bg-blue-600 hover:text-white font-black uppercase tracking-[0.2em] text-[11px] rounded-full transition-all active:scale-95 shadow-2xl">Confirm Configuration</button>
          </footer>
        </main>
      </div>
    </div>
